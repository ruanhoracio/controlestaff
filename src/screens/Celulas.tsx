...
import { RODIZIOS, type RodizioId } from '../lib/config'
import { proximaCelula, proximoEsocial, responsavelDaVez } from '../lib/rodizio'
...
export default function Celulas() {
  ...
  // O eSocial tem fila própria: avança a cada designação, de qualquer rodízio.
  const esocialDaVez = proximoEsocial(desigCelulas, equipe)

  async function designar(rodizioId: RodizioId, pulada: boolean) {
    const empresa = (empresaPorRodizio[rodizioId] ?? '').trim()
  ...
  return (
    <div>
      <TituloTela
        ...
        descricao="Cada rodízio anda por conta. Designou, a vez avança sozinha — inclusive a do eSocial."
      />

      {erro && <p className="mb-4 text-xs text-red-600 bg-red-50 border border-red-100 rounded-2xl px-4 py-3">{erro}</p>}

      {/* Fila do eSocial, comum a todos os rodízios */}
      <div className="glass-card p-5 mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 font-light">
            Fila própria, avança junto com qualquer designação.
          </p>
        </div>
        {esocialDaVez ? (
          <div className="flex items-center gap-2">
            <span className="badge-neutral">próximo</span>
            <span className="text-lg text-slate-950 tracking-tight">{esocialDaVez}</span>
          </div>
        ) : (
          <span className="text-xs text-slate-400 font-light">cadastre a fila de eSocial em Equipe</span>
        )}
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-8">
        {RODIZIOS.map((rod) => {
          ...
          return (
            <div key={rod.id} className="glass-card p-6">
              ...
              {posicao ? (
                <div className="rounded-2xl bg-gradient-to-b from-blue-500 to-blue-600 border border-blue-700 shadow-btn px-5 py-4 mb-4">
                  ...
                  <p className="text-xs text-blue-100 font-light mt-0.5">
                    {/* No rodízio de ergonomistas a posição já É a pessoa; não repetir */}
                    {[responsavel !== posicao ? responsavel : '', esocialDaVez ? `eSocial: ${esocialDaVez}` : '']
                      .filter(Boolean)
                      .join(' · ') || '—'}
                  </p>
                </div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) => setEmpresaPorRodizio((m) => ({ ...m, [rod.id]: e.target.value }))}
                  onKeyDown={(e) => {
                ...
                <button
                  ...
                  disabled={ocupado || !posicao || !(empresaPorRodizio[rod.id] ?? '').trim()}
